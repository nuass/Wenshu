...
export async function handleFeishuMessage(params: {
  ...
  try {
    ...
    // ── 题目推送指令拦截 ──────────────────────────────────────────
    // 识别 "@机器人 请给 @用户 发一道题" 并调用 auto_send 推送引擎
    if (ctx.mentionedBot) {
      const pushTarget = parseQuestionPushTarget(ctx.content, ctx.senderOpenId);
      if (pushTarget) {
        await handleQuestionPush({
          cfg,
          targetOpenId: pushTarget,
          replyToChatId: ctx.chatId,
